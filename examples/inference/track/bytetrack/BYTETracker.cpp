...
byte_track::BYTETracker::BYTETracker(const int& frame_rate,
    ...
    : track_thresh_(track_thresh),
      ...
      max_time_remove_(5 * max_time_lost_),
      frame_id_(0),
      track_id_count_(0)
{
}
...
std::vector<byte_track::BYTETracker::STrackPtr> byte_track::BYTETracker::update(const std::vector<Object>& objects)
{
    ...
    removed_stracks_.erase(std::remove_if(removed_stracks_.begin(), removed_stracks_.end(),
                                          [this](const STrackPtr& s) {
                                              return this->frame_id_ - s->getFrameId() > this->max_time_remove_;
                                          }),
                                                                                      ...
                                                                                      const std::vector<STrackPtr>& b_tlist) const
{
    ...
    for (size_t i = 0; i < b_tlist.size(); i++) {
        ...
        if (exists.emplace(tid, 1).second) {
                                          ...
                                          bool return_cost) const
{
    ...
    std::vector<std::vector<double>> cost_data(n, std::vector<double>(n));
    std::vector<double*> cost_ptr(n);
    for (int i = 0; i < n; i++)
        cost_ptr[i] = cost_data[i].data();
