...
enum MEDIA_BUFFER_TYPE : int32_t {
    BUFFER_TYPE_VIDEO,
    BUFFER_TYPE_AUDIO,
    BUFFER_TYPE_ETC
};
...
public:
    ...
    /**
     * Copy frame metadata without copying payload or buffer ownership state.
     * Resource identity such as storage, active ranges, pool index, reference
     * count, callbacks, private_data and extra_data remains unchanged.
     */
    virtual void copyMetadata(const MediaBuffer& other);
