...
class FFMEDIA_API ModuleMedia : public MediaProducer,
...
public:
    ...
    ~ModuleMedia() override;

    ModuleMedia(const ModuleMedia&) = delete;
    ModuleMedia& operator=(const ModuleMedia&) = delete;
    ModuleMedia(ModuleMedia&&) = delete;
    ModuleMedia& operator=(ModuleMedia&&) = delete;
...
public:
    ...
    /** Deprecated no-op. Retain the shared_ptr directly instead. */
    [[deprecated("MediaBuffer lifetime follows std::shared_ptr; retain it directly")]] static int holdOutputBuffer(const std::shared_ptr<MediaBuffer>& obuf);

    /** Deprecated no-op. Release the retained shared_ptr instead. */
    [[deprecated("MediaBuffer lifetime follows std::shared_ptr; reset it directly")]] static int releaseOutputBuffer(const std::shared_ptr<MediaBuffer>& obuf);
     ...
     */
    const ImagePara& getInputImagePara() const;
     ...
     * @return {ImagePara}  返回对象的输出数据的图像参数。
     */
    const ImagePara& getOutputImagePara() const;
    ...
    const std::vector<MediaChannelRequirement>&
    getInputMediaChannelRequirements() const;
     ...
     */
    MediaStatus getModuleStatus() const;
    /**
     ...
     * @return {*}
     */
    void setSynchronize(std::shared_ptr<Synchronize> syn);
    ...
    void receiveMediaBuffer(MediaBufferContext&& context) final;
    /**
...
protected:
    using OutputBufferList = std::vector<std::shared_ptr<MediaBuffer>>;
    using BufferRecycleHandler = std::function<void(
        const std::shared_ptr<MediaBuffer>&)>;

    struct OutputBufferPool {
        OutputBufferList buffers;
        OutputBufferList rotation_buffers;
        BufferRecycleHandler recycle_handler;
    };

    enum ConsumeResult : int32_t {
        CONSUME_SUCCESS = 0,
...
protected:
    ...
    int commitOutputBufferPool(OutputBufferPool&& pool);
    int bindOutputBufferRecycleHandler(BufferRecycleHandler handler);
    void clearOutputBufferPool();
    size_t outputBufferPoolSize() const noexcept;
    std::shared_ptr<MediaBuffer> outputBufferAt(size_t index) const;
    OutputBufferList availableOutputBuffers() const;
    bool outputBuffersInFlight();

    ModuleType getModuleType() const noexcept;
    void setModuleType(ModuleType type) noexcept;
    void setMediaType(MEDIA_BUFFER_TYPE type) noexcept;
    size_t getConfiguredBufferSize() const noexcept;
    bool isInitialized() const noexcept;
    void setInitialized(bool value = true) noexcept;
    Synchronize* synchronizer() const noexcept;
    void setClearCachePolicy(bool enabled) noexcept;
    std::unique_lock<std::mutex> lockConfiguration();
    /** Call only while holding lockConfiguration(). */
    bool isWorkerActive() const noexcept;

    int setOutputBufferQueueHead(const std::shared_ptr<MediaBuffer>& buffer);

    virtual void bufferReleaseCallBack(const std::shared_ptr<MediaBuffer>& buffer);

    void setModuleStatus(const MediaStatus& status);
    virtual bool setup();
...
private:
    struct OutputPoolEpoch;
    struct RecycleState;
    struct FrameRecycleDeleter;

    void resetModule();

    void rebuildOutputBufferPool();
    ...
    std::shared_ptr<MediaBuffer>& waitProduceBufferInternal(
        bool allow_stopping, uint32_t timeout_ms);
    void abandonOutputReservation();
    void ensureOutputBufferPool();
    std::shared_ptr<OutputPoolEpoch> makeOutputPoolEpoch(
        const OutputBufferList& owner_buffers,
        const OutputBufferList& rotation_buffers,
        BufferRecycleHandler recycle_handler);
    void installOutputPoolEpoch(
        std::shared_ptr<OutputPoolEpoch> next_epoch) noexcept;
    void drainRecycleEvents();
    bool invokeBufferReleaseCallbackNoexcept(
        const std::shared_ptr<MediaBuffer>& buffer,
        const BufferRecycleHandler& recycle_handler) noexcept;
    void setModuleAbnormalNoexcept() noexcept;
    void closeRecycleState();
    bool outputPoolMatchesBuffers() const;
...
/**
 * Preferred construction helper for external modules.
 *
 * The custom deleter stops the worker while the most-derived object is still
 * alive, before its destructor starts releasing device or codec resources.
 * Module construction is a cold path; this intentionally favors lifecycle
 * safety over make_shared's single allocation.
 */
template <typename Module, typename... Args>
std::shared_ptr<Module> makeMediaModule(Args&&... args)
{
    static_assert(std::is_base_of<ModuleMedia, Module>::value,
                  "Module must derive from ModuleMedia");
    return std::shared_ptr<Module>(
        new Module(std::forward<Args>(args)...),
        [](Module* module) {
            if (!module)
                return;
            module->stop();
