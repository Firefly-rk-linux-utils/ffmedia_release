...
class FFMEDIA_API DrmDisplayPlane : public std::enable_shared_from_this<DrmDisplayPlane>
{
public:
...
public:
     ...
     * @param {int} _screen_index       DRM card编号，对应/dev/dri/cardN。
    ...
    /**
     * @description: 在setup前设置完整图层参数，供connector/CRTC链路选择使用。
     * @return {int} 成功返回0，参数与已初始化图层冲突时返回负数。
     */
    int setPlanePara(uint32_t fmt, uint32_t plane_id, PLANE_TYPE plane_type,
                     uint32_t plane_zpos, uint32_t plane_linear,
                     uint32_t conn_id = 0);
...
private:
    ...
    bool savePlaneState();
    bool restorePlaneState();
    int drmFindPlane(uint32_t requested_plane_id = 0);
    int drmCreateFb(const std::shared_ptr<VideoBuffer>& buffer);
    int drmRemoveFb(int fb_id);
...
private:
    ...
    SavedPlaneState saved_plane_state;
    std::unordered_map<uint32_t, uint32_t> imported_fb_handles;
    std::unordered_map<uint32_t, uint32_t> imported_handle_refs;
    std::mutex imported_fb_mtx;
    std::unordered_set<ModuleDrmDisplay*> windows;
    std::unique_ptr<std::thread> display_thread;
    std::atomic_bool display_thread_exit;
};

class FFMEDIA_API ModuleDrmDisplay : public ModuleMedia
{
...
public:
    ...
    /**
     * @description: 设置图层显示模式，该接口应在对象初始化前调用。
