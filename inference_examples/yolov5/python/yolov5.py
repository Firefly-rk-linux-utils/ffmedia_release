...
def draw(image, boxes, scores, classes, ratio=(1.0,1.0)):
    for box, score, cl in zip(boxes, scores, classes):
        top, left, right, bottom = box
        # print('class: {}, score: {}'.format(CLASSES[cl], score))
        # print('box coordinate left,top,right,down: [{}, {}, {}, {}]'.format(top, left, right, bottom))
        top = int(top // ratio[1])
        left = int(left // ratio[0])
        right = int(right // ratio[0])
        bottom = int(bottom // ratio[1])
...
def inf_task(inf, media_buffer, ratio=(1.0,1.0)):

    #start_time = time.perf_counter()
    # Inference
    outputs = inf.inference(media_buffer)
    #end_time = time.perf_counter()
    #print(f"inference run use: {end_time - start_time:.6f} s")
