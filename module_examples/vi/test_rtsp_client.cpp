...
int main(int argc, char** argv)
{
    ...
    rtsp_c->setOutputDataCallback(nullptr, output_callback);
    rtsp_c->setStatusChangeCallback(nullptr, status_change_callback);
    ret = rtsp_c->init();
    if (ret < 0) {
    ...
    auto v_extra = rtsp_c->getExtraBuffer(BUFFER_TYPE_VIDEO);
    if (v_extra) {
    ...
    auto a_extra = rtsp_c->getExtraBuffer(BUFFER_TYPE_AUDIO);
    if (a_extra) {
